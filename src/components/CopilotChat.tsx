...
import { useSiagaBanjarPulse } from '@/hooks/useSiagaBanjarPulse';
...
interface CopilotChatProps { userLocation: Location | null; facilities?: any[]; language?: 'en' | 'id'; }

const LANGUAGES = [
  { code: 'id', label: 'Bahasa Indonesia', voice: 'id-ID' },
  { code: 'en', label: 'English', voice: 'en-US' },
];

const CopilotChat = ({ userLocation, facilities = [], language: propLanguage = 'id' }: CopilotChatProps) => {
  ...
  const { pulse, clearInterjection } = useSiagaBanjarPulse(userLocation);
  ...
  useEffect(() => {
    if (userLocation) {
      fetch(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${userLocation.lat}&longitude=${userLocation.lng}&localityLanguage=${language}`)
  ...
  const toggleListening = () => {
    ...
    if (isListening) {
      ...
      try { 
        ...
        const errTitle = language === 'en' ? "Error starting mic" : "Gagal memulai mik";
        const errDesc = err.message || (language === 'en' ? "Failed to start listening." : "Gagal mulai mendengarkan.");
        toast({ title: errTitle, description: errDesc, variant: "destructive" });
        setIsListening(false);
        isListeningRef.current = false;
      }
    }
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    const langLabel = LANGUAGES.find(l => l.code === language)?.label || 'Bahasa Indonesia';
...
IMPORTANT: Use ONLY this real data above when answering. Do NOT make up or estimate any values. Present this data clearly and conversationally.`;
          console.log(`🌤️ Siaga Banjar: Fetched live weather for ${weatherCity} (${weatherLat?.toFixed(2)}, ${weatherLng?.toFixed(2)})`);
        }
      } catch (err) {
    ...
    const systemPrompt = `Anda adalah asisten cerdas untuk manajemen bencana dan prediksi cuaca di Banjarnegara, Jawa Tengah, Indonesia.
Aturan:
- Balas HANYA dalam ${langLabel}.
- Jadilah sangat membantu dan berikan detail mendalam untuk panduan keselamatan atau protokol bencana di wilayah Banjarnegara.
- Gunakan data geografi Banjarnegara: Kabupaten yang rawan tanah longsor dan banjir sungai Serayu.
- Untuk langkah darurat, gunakan bullet points agar jelas.
- Tidak perlu salam pembuka yang berlebihan. Hanya fakta dan saran praktis.
- Anda memiliki akses ke data cuaca real-time dan fasilitas terdekat.
...
- User location: ${locationName || 'Banjarnegara, Indonesia'}.${weatherContext}`;
    try {
      let aiText = '';
      ...
      const errTitle = language === 'en' ? 'Connection error' : 'Kesalahan koneksi';
      const errDesc = language === 'en' ? 'Could not reach Siaga Banjar.' : 'Tidak dapat menghubungi Siaga Banjar.';
      toast({ title: errTitle, description: errDesc, variant: 'destructive' });
  ...
  const handleActionClick = (token: string) => {
    ...
    console.log(`📡 Siaga Banjar UI Action triggered: ${cmd} (${value})`);
  ...
  const quickPrompts = language === 'id' ? [
    ...
    'Langkah keselamatan banjir',
    'Kontak darurat',
    'Protokol gempa bumi',
    'Tanda peringatan siklon',
    'Rute evakuasi',
    'Tips pertolongan pertama',
    'Langkah darurat kebakaran',
  ] : [
    'Nearest hospital',
    'Flood safety steps',
    'Emergency contacts',
    ...
    return (
      <div className="h-full flex flex-col" style={bgStyle}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6">
          ...
          <div className="text-center space-y-1 mt-2">
            <p className="text-base font-semibold text-slate-600">
              {isListening ? (language === 'en' ? '🎙 Listening...' : '🎙 Mendengarkan...') : isSpeaking ? (language === 'en' ? '🔊 Speaking...' : '🔊 Berbicara...') : loading ? (language === 'en' ? '💭 Thinking...' : '💭 Berpikir...') : (language === 'en' ? 'Tap mic to speak' : 'Ketuk mic untuk berbicara')}
            </p>
            {isListening && (
              <p className="text-[10px] text-slate-400 mt-0.5 uppercase tracking-wider">
                {language === 'en' ? 'Speak now • Auto-sends when you pause' : 'Bicaralah sekarang • Kirim otomatis saat Anda berhenti'}
              </p>
            )}
        ...
        <div className="pb-8 px-6">
          ...
          <p className="text-center text-[10px] text-slate-400/60 mt-5 font-medium">Asisten AI · Dasbor Prediktif Cuaca</p>
        </div>
      </div>
    );
  }
        ...
        {messages.length === 0 ? (
          <div className="pt-2 space-y-5">
            <div className="flex flex-col items-center py-6">
              ...
              <h2 className="text-lg font-bold text-slate-700 -mt-2">{language === 'en' ? 'Ask Assistant' : 'Tanya Asisten'}</h2>
              <p className="text-sm text-slate-400 mt-1">
                {locationName ? (language === 'en' ? `Serving ${locationName}` : `Melayani ${locationName}`) : (language === 'en' ? 'Your disaster management AI' : 'AI manajemen bencana Anda')}
              </p>
            </div>
              ...
              {msg.role === 'user' ? (
                ...
                <div className="max-w-[90%] space-y-2">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Siaga Banjar</span>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => speakText(msg.content)}
                      ...
                      <Volume2 className="w-3 h-3" /> {language === 'en' ? 'Listen' : 'Dengarkan'}
                    </button>
                  </div>
      ...
      <div className="px-4 pb-5 pt-3 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 bg-white/60 backdrop-blur-md border border-white/90 rounded-2xl px-4 py-3 shadow-sm">
          ...
          <input
            ...
            placeholder={attachedImage ? 'Tanyakan tentang gambar ini...' : `Tanya Siaga Banjar${locationName ? ` tentang ${locationName}` : ''}...`}
        ...
        <p className="text-center text-[10px] text-slate-400/60 mt-2 font-medium">Siaga Banjar AI · EWS Banjarnegara</p>
      </div>
    </div>
  );
};
