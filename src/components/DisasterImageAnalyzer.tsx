...
interface DisasterImageAnalyzerProps {
  language?: 'en' | 'id';
}

const SEVERITY_CONFIG = {
  none:         { color: 'from-emerald-500 to-green-600',  bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', text: 'text-emerald-600', label: { en: 'No Damage', id: 'Tidak Ada Kerusakan' }, emoji: '✅' },
  minor:        { color: 'from-sky-500 to-blue-600',       bg: 'bg-sky-500/10',     border: 'border-sky-500/30',     text: 'text-sky-600',     label: { en: 'Minor', id: 'Ringan' },     emoji: '🔵' },
  moderate:     { color: 'from-amber-500 to-orange-600',   bg: 'bg-amber-500/10',   border: 'border-amber-500/30',   text: 'text-amber-600',   label: { en: 'Moderate', id: 'Sedang' },  emoji: '🟠' },
  severe:       { color: 'from-red-500 to-rose-600',       bg: 'bg-red-500/10',     border: 'border-red-500/30',     text: 'text-red-600',    label: { en: 'Severe', id: 'Berat' },    emoji: '🔴' },
  catastrophic: { color: 'from-purple-600 to-red-700',     bg: 'bg-purple-500/10',  border: 'border-purple-500/30',  text: 'text-purple-600',  label: { en: 'Catastrophic', id: 'Sangat Berat/Bencana' }, emoji: '⚠️' },
};

const DisasterImageAnalyzer: React.FC<DisasterImageAnalyzerProps> = ({ language = 'id' }) => {
  const t = {
    en: {
      title: "Disaster Image Analyzer",
      subtitle: "AI-powered damage assessment via Llama Vision + HF",
      uploadTitle: "Upload disaster image",
      uploadDesc: "Drag & drop or tap to select • JPG, PNG, WebP • Max 10MB",
      gallery: "Gallery",
      camera: "Camera",
      analyzing: "Analyzing image...",
      classifying: "Classifying damage severity",
      analyzeBtn: "Analyze Damage",
      severityTitle: (label: string) => `${label} Damage`,
      confidence: "Confidence",
      aiAssessment: "AI Assessment",
      recommendedActions: "Recommended Actions",
      classificationDetails: "Classification Details",
      anotherImage: "Analyze Another Image",
      poweredBy: "Powered by Llama 4 Scout Vision",
      footerDesc: "AI vision model analyzes your image directly to assess disaster damage. Images are processed via API and not stored. For emergencies, call 112.",
      invalidFile: "Invalid file",
      invalidFileDesc: "Please upload an image file (JPG, PNG, WebP).",
      fileTooLarge: "File too large",
      fileTooLargeDesc: "Maximum file size is 10MB.",
      analysisFailed: "Analysis failed",
      analysisFailedDesc: "Could not analyze the image. Please try again.",
      aiPrompt: "You are a disaster damage assessment AI. Analyze this image carefully and respond with ONLY a valid JSON object. Focus on Indonesian context if relevant.",
    },
    id: {
      title: "Analis Gambar Bencana",
      subtitle: "Penilaian kerusakan berbasis AI melalui Llama Vision + HF",
      uploadTitle: "Unggah gambar bencana",
      uploadDesc: "Seret & lepas atau ketuk untuk memilih • JPG, PNG, WebP • Maks 10MB",
      gallery: "Galeri",
      camera: "Kamera",
      analyzing: "Menganalisis gambar...",
      classifying: "Mengklasifikasi tingkat kerusakan",
      analyzeBtn: "Analisis Kerusakan",
      severityTitle: (label: string) => `Kerusakan ${label}`,
      confidence: "Tingkat Keyakinan",
      aiAssessment: "Penilaian AI",
      recommendedActions: "Tindakan yang Disarankan",
      classificationDetails: "Detail Klasifikasi",
      anotherImage: "Analisis Gambar Lain",
      poweredBy: "Ditenagai oleh Llama 4 Scout Vision",
      footerDesc: "Model visi AI menganalisis gambar Anda secara langsung untuk menilai kerusakan bencana. Gambar diproses melalui API dan tidak disimpan. Untuk keadaan darurat, hubungi 112.",
      invalidFile: "Berkas tidak valid",
      invalidFileDesc: "Silakan unggah berkas gambar (JPG, PNG, WebP).",
      fileTooLarge: "Berkas terlalu besar",
      fileTooLargeDesc: "Ukuran berkas maksimum adalah 10MB.",
      analysisFailed: "Analisis gagal",
      analysisFailedDesc: "Tidak dapat menganalisis gambar. Silakan coba lagi.",
      aiPrompt: "Anda adalah AI penilai kerusakan bencana. Analisis gambar ini dengan cermat dan berikan respons HANYA dalam objek JSON yang valid. Fokus pada konteks Indonesia jika relevan.",
    }
  };
  ...
  const handleFile = useCallback((file: File) => {
    if (!file.type.startsWith('image/')) {
      toast({ title: l.invalidFile, description: l.invalidFileDesc, variant: 'destructive' });
      return;
    }
          ...
          messages: [
            {
              ...
              content: [
                ...
                {
                  type: 'text',
                  text: `${l.aiPrompt} (no markdown, no explanation, return labels and description in ${language === 'id' ? 'Indonesian' : 'English'}):
{
...
- recommendations should be actionable safety advice`
      ...
      toast({ title: l.analysisFailed, description: err.message || l.analysisFailedDesc, variant: 'destructive' });
    } finally {
      setAnalyzing(false);
    }
  };
  ...
  const processResults = (data: any, vitResults: AnalysisResult[]) => {
    ...
    const summary = severity === 'none'
      ? (language === 'en' ? `The image appears to show a normal scene with no visible disaster damage (${confidencePct}% confidence).` : `Gambar tampak menunjukkan pemandangan normal tanpa kerusakan bencana yang terlihat (tingkat keyakinan ${confidencePct}%).`)
      : (language === 'en' ? `AI vision detected **${severity} ${disasterType} damage** — ${description} (${confidencePct}% confidence).` : `Visi AI mendeteksi **kerusakan ${disasterType} tingkat ${severity}** — ${description} (tingkat keyakinan ${confidencePct}%).`);
  ...
  const getRecommendations = (severity: DamageReport['overallSeverity'], label: string): string[] => {
    ...
    if (severity === 'none') {
      return language === 'en' 
        ? ['No immediate action required.', 'Continue monitoring local alerts via the Early Warnings tab.']
        : ['Tidak ada tindakan segera yang diperlukan.', 'Lanjutkan memantau peringatan lokal melalui tab Peringatan Dini.'];
    }
    ...
    if (language === 'en') {
      ...
      if (label.toLowerCase().includes('flood') || label.toLowerCase().includes('banjir')) {
      ...
      if (label.toLowerCase().includes('earthquake') || label.toLowerCase().includes('collapsed') || label.toLowerCase().includes('gempa')) {
        base.push('Check for gas leaks. DO NOT use open flames.');
        base.push('Be prepared for aftershocks.');
      }
      if (label.toLowerCase().includes('fire') || label.toLowerCase().includes('kebakaran')) {
        base.push('Call fire brigade: 113. Stay upwind from smoke.');
      ...
      base.push('Dokumentasikan kerusakan dengan beberapa foto dari berbagai sudut.');
      if (severity === 'severe' || severity === 'catastrophic') {
        base.push('🚨 EVAKUASI area segera jika Anda berada di dekatnya.');
        base.push('Hubungi layanan darurat: 112 atau 115 (Basarnas).');
        base.push('JANGAN memasuki struktur yang rusak — risiko runtuh.');
      }
      if (severity === 'moderate') {
        base.push('Hindari area yang terpengaruh sampai pihak berwenang mengonfirmasi keamanan.');
        base.push('Laporkan kerusakan ke BPBD (Badan Penanggulangan Bencana Daerah).');
      }
      if (label.toLowerCase().includes('flood') || label.toLowerCase().includes('banjir')) {
        base.push('Pindah ke tempat yang lebih tinggi. Hindari berjalan melalui air banjir.');
        base.push('Matikan listrik di saklar utama jika air mulai naik.');
      }
      if (label.toLowerCase().includes('earthquake') || label.toLowerCase().includes('collapsed') || label.toLowerCase().includes('gempa')) {
        base.push('Periksa kebocoran gas. JANGAN gunakan api terbuka.');
        base.push('Bersiaplah untuk gempa susulan.');
      }
      if (label.toLowerCase().includes('fire') || label.toLowerCase().includes('kebakaran')) {
        base.push('Hubungi pemadam kebakaran: 113. Tetap berada di sisi yang tidak terkena asap.');
        base.push('Jangan mencoba memasuki kembali bangunan yang terbakar.');
      }
      if (severity === 'minor') {
        base.push('Nilai integritas struktural sebelum masuk kembali.');
        base.push('Laporkan kerusakan ke kantor kelurahan/kecamatan setempat.');
      }
    }
    return base;
  };
    ...
    <div className="h-full overflow-y-auto p-3 pt-4 pb-20 md:pb-3 sm:p-6 md:pt-6">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-lg font-bold tracking-tight text-[#2d5a27] dark:text-[#c8d8b8]">{l.title}</h2>
            <p className="text-xs text-[#5a6b4f] dark:text-[#aab3a3]">{l.subtitle}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 mt-2">
          <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-[#2d5a27]/8 dark:bg-[#c8d8b8]/10 text-[#2d5a27] dark:text-[#c8d8b8] text-[10px] font-semibold rounded-full border border-[#2d5a27]/15 dark:border-[#c8d8b8]/20">
            🦙 {language === 'en' ? 'Llama 4 Scout Vision' : 'Visi Llama 4 Scout'}
          </span>
      ...
      {!imagePreview ? (
        ...
        >
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground mb-1">{l.uploadTitle}</p>
              <p className="text-xs text-muted-foreground">{l.uploadDesc}</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 mt-6">
            ...
            >
              <Upload className="w-4 h-4" /> {l.gallery}
            </button>
            <button
            ...
            >
              <Camera className="w-4 h-4" /> {l.camera}
            </button>
          </div>
        </div>
      ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="relative rounded-2xl overflow-hidden border border-[#c4b99a]/30 shadow-sm">
            ...
            {analyzing && (
              <div className="absolute inset-0 bg-black/30 backdrop-blur-sm flex flex-col items-center justify-center gap-3">
                ...
                <p className="text-white text-sm font-semibold">{l.analyzing}</p>
                <p className="text-white/60 text-xs">{l.classifying}</p>
              </div>
            )}
          </div>
          ...
          {!report && !analyzing && (
            ...
            >
              <Zap className="w-4 h-4" /> {l.analyzeBtn}
            </button>
          )}
          ...
          {report && sev && (
            <div className="space-y-3">
              ...
              <div className={`p-4 rounded-2xl border ${sev.border} ${sev.bg}`}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className={`text-base font-bold ${sev.text}`}>{l.severityTitle(sev.label[language])}</h3>
                      <p className="text-xs text-muted-foreground">{l.confidence}: {Math.round(report.confidence * 100)}%</p>
                    </div>
                  </div>
              ...
              <div className="p-4 rounded-2xl bg-[#f5f0e8]/50 dark:bg-card/50 border border-[#c4b99a]/20 dark:border-white/10">
                <h4 className="text-xs font-semibold text-[#5a6b4f] dark:text-[#aab3a3] uppercase tracking-wider mb-2">{l.aiAssessment}</h4>
              ...
              <div className="p-4 rounded-2xl bg-[#f5f0e8]/50 dark:bg-card/50 border border-[#c4b99a]/20 dark:border-white/10">
                <h4 className="text-xs font-semibold text-[#5a6b4f] dark:text-[#aab3a3] uppercase tracking-wider mb-3 flex items-center gap-1.5">
                  <AlertTriangle className="w-3.5 h-3.5" /> {l.recommendedActions}
                </h4>
              ...
              <div className="rounded-2xl bg-[#f5f0e8]/50 dark:bg-card/50 border border-[#c4b99a]/20 dark:border-white/10 overflow-hidden">
                ...
                >
                  <span>{l.classificationDetails} ({report.categories.length})</span>
              ...
              >
                <RotateCcw className="w-4 h-4" /> {l.anotherImage}
              </button>
            </div>
          )}
        </div>
      ...
      <div className="mt-6 p-3 rounded-xl bg-[#f5f0e8]/40 dark:bg-[#d4c5b0]/5 border border-[#c4b99a]/15 dark:border-[#d4c5b0]/15">
        <div className="flex items-start gap-2">
          ...
          <p className="text-[10px] text-[#8b7355] dark:text-[#d4c5b0] leading-relaxed">
            <strong>{l.poweredBy}</strong> + Hugging Face ViT. {l.footerDesc}
          </p>
        </div>
      </div>
    </div>
