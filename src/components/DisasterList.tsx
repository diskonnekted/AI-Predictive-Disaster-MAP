...
const DisasterList: React.FC<DisasterListProps> = ({ disasters, onDisasterClick, loading, userLocation, language = 'id' }) => {
  const [expandedItems, setExpandedItems] = useState<Set<string>>(new Set());

  const t = {
    en: {
      noActive: "No Active Disasters",
      noActiveDesc: "No verified disasters with official reports found. Only disasters with real report links from USGS or GDACS are shown.",
      noRegional: "No Regional Activity",
      scanning: "Scanning telemetry for official verification...",
      docs: "DOCS",
      ...
      prob: "prob",
      regionalTelemetry: "REGIONAL TELEMETRY",
      localized: "LOCALIZED",
      nationalFeed: "NATIONAL VERIFIED FEED",
      total: "TOTAL",
      neuralRisk: "NEURAL RISK PREDICTIONS",
      mlInference: "ML INFERENCE",
      unknown: "Unknown",
      daysAbbr: "d"
    },
    id: {
      noActive: "Tidak Ada Bencana Aktif",
      noActiveDesc: "Tidak ditemukan bencana terverifikasi dari laporan resmi. Hanya bencana dengan tautan laporan nyata dari USGS atau lembaga rujukan yang ditampilkan.",
      noRegional: "Tidak Ada Aktivitas Regional",
      scanning: "Memindai telemetri untuk verifikasi resmi...",
      docs: "DOKUMEN",
      telemetryMap: "PETA TELEMETRI",
      magnitude: "Magnitudo",
      coordinates: "Koordinat",
      eventId: "ID Peristiwa",
      expectedIn: "Diperkirakan dalam",
      prob: "peluang",
      regionalTelemetry: "TELEMETRI REGIONAL",
      localized: "TERDETEKSI LOKAL",
      nationalFeed: "UMPAN NASIONAL TERVERIFIKASI",
      total: "TOTAL",
      neuralRisk: "PREDIKSI RISIKO NEURAL (AI)",
      mlInference: "INFERENSI ML",
  ...
  if (verifiedDisasters.length === 0) {
    return (
      <Card className="glass p-6 sm:p-8 text-center">
        ...
        <h3 className="font-semibold text-base sm:text-lg mb-2">{l.noActive}</h3>
    ...
    if (disastersList.length === 0) {
      return (
        <Card className="bg-white/40 dark:bg-slate-900/40 border-slate-200/50 dark:border-white/5 p-8 text-center rounded-2xl">
          ...
          <h3 className="font-black text-[10px] tracking-[0.2em] mb-1 uppercase text-primary/60">{l.noRegional}</h3>
          <p className="text-[10px] text-primary/40 uppercase font-bold">{l.scanning}</p>
        </Card>
      );
    }
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-4 mt-3 text-[10px] text-slate-500 dark:text-slate-600 font-bold uppercase tracking-wider">
                            <div className="flex items-center gap-1.5 bg-slate-100/50 dark:bg-slate-900/40 px-2 py-1 rounded-lg border border-slate-200/50 dark:border-white/5">
                              ...
                              <span>{disaster.time ? new Date(disaster.time).toLocaleString(language === 'id' ? 'id-ID' : 'en-IN', { dateStyle: 'medium', timeStyle: 'short' }) : l.unknown}</span>
                            </div>
                      ...
                      <div className="flex items-center gap-2 pt-2">
                        ...
                        >
                          <a href={disaster.url} target="_blank" rel="noopener noreferrer">
                            ...
                            {getSourceName(disaster.url!)} {l.docs}
                          </a>
                        </Button>
                        <Button
                          size="sm"
                        ...
                        <div className="border-t border-border/20 pt-3 space-y-3">
                          {disaster.magnitude && (
                            <div className="flex items-center gap-2">
                              <span className="font-medium text-xs sm:text-sm">{l.magnitude}:</span>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm">
                            <div>
                              <span className="font-medium">{l.coordinates}:</span>
                              <p className="text-muted-foreground font-mono text-[11px]">
                                {disaster.location.lat.toFixed(4)}, {disaster.location.lng.toFixed(4)}
                              </p>
                            </div>
                            <div>
                              <span className="font-medium">{l.eventId}:</span>
                            ...
                            <div className="flex items-center gap-2 flex-wrap text-xs">
                              {disaster.timeframeDays !== undefined && (
                                <Badge variant="outline" className="bg-purple-500/10 border-purple-500/30 text-[10px]">
                                  {l.expectedIn} {disaster.timeframeDays}{l.daysAbbr}
                                </Badge>
                              )}
                              {disaster.probability !== undefined && (
                                <Badge variant="outline" className="bg-purple-500/10 border-purple-500/30 text-[10px]">
                                  {(disaster.probability * 100).toFixed(0)}% {l.prob}
                                </Badge>
                              )}
                            </div>
                          )}
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-[10px] font-black tracking-[0.3em] text-slate-500 uppercase flex items-center gap-2">
            <Globe className="h-4 w-4" /> {l.nationalFeed}
          </h2>
          <Badge variant="outline" className="bg-slate-100/50 dark:bg-slate-900/40 text-[9px] font-black uppercase tracking-widest border-slate-200/50 dark:border-white/5 py-1 px-3">
            {verifiedDisasters.length} {l.total}
          </Badge>
        </div>
      ...
      {predictions.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-[10px] font-black tracking-[0.3em] text-slate-500 uppercase flex items-center gap-2">
              <Database className="h-4 w-4" /> {l.neuralRisk}
            </h2>
          </div>
