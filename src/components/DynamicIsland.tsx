...
const DynamicIsland: React.FC<DynamicIslandProps> = ({ userLocation, language = 'id' }) => {
  ...
  const [cityName, setCityName] = useState<string>(language === 'en' ? 'Detecting...' : 'Mendeteksi...');
  ...
  useEffect(() => {
    if (userLocation) {
      ...
      fetch(
        `https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${userLocation.lat}&longitude=${userLocation.lng}&localityLanguage=${language}`
      )
        .then(res => res.json())
        .then(data => {
          const city =
        ...
        .catch((err) => {
          ...
          setCityName(language === 'en' ? 'Location detected' : 'Lokasi terdeteksi');
        });
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <div className="flex flex-col min-w-0">
              ...
              {userLocation && cityName !== (language === 'en' ? 'Detecting...' : 'Mendeteksi...') && (
            ...
            {/* Weather Info */}
            {weather && cityName !== (language === 'en' ? 'Detecting...' : 'Mendeteksi...') && (
              <>
