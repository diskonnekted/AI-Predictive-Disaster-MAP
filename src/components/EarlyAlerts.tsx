...
const EarlyAlerts: React.FC<EarlyAlertsProps> = ({ userLocation, language }) => {
  const t = {
    ...
    id: {
      title: "Peringatan Dini Bencana & Analisis Risiko",
      subtitle: "Intelijen Langsung",
      compositeRisk: "Risiko Area Gabungan",
      analyzing: "Menganalisis sinyal waktu nyata...",
      noAlerts: "Tidak ada ancaman bencana langsung yang terdeteksi untuk lokasi Anda saat ini.",
      probability: "Probabilitas",
      recommendation: "Rekomendasi",
      threatLevel: "Tingkat Ancaman",
      fetchingWeather: "Mengambil data cuaca",
      fetchingPrecip: "Mengambil prakiraan presipitasi",
      fetchingSeismic: "Menanyakan aktivitas seismik",
      checkingGlobal: "Memeriksa peringatan global",
      measuringAQI: "Mengukur kualitas udara",
      loadingAI: "Memuat Model Neural ML",
      runningML: "Menjalankan inferensi ML",
      emergency: "Darurat",
      warning: "Peringatan",
      watch: "Pantauan",
      advisory: "Saran",
      alertTriggered: "Peringatan Dipicu",
      fetchFailed: "Pengambilan Gagal",
      allClear: "Semua Aman",
      enableLocation: "Aktifkan lokasi untuk menerima peringatan dini untuk banjir, gempa bumi, dan cuaca ekstrem.",
      warnings: "Peringatan Dini",
    }
  };
  ...
  const fetchAlerts = useCallback(async () => {
    ...
    try {
      const data = await fetchEarlyAlertsLocal(lat, lng, language, (phase) => {
  ...
  const handleDownloadReport = () => {
    ...
    element.innerHTML = `
      <div style="border-bottom: 3px solid #0f172a; padding-bottom: 24px; margin-bottom: 40px; display: flex; justify-content: space-between; align-items: flex-end;">
        <div>
          <h1 style="margin: 0; color: #0f172a; font-size: 32px; letter-spacing: -0.05em; font-weight: 800;">DASBOR<span style="color: #64748b;">PREDIKTIF</span></h1>
          <p style="margin: 4px 0 0 0; color: #64748b; font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.1em;">Arsitektur Pendukung Keputusan Bencana</p>
        </div>
      ...
      <div style="margin-top: 80px; padding-top: 20px; border-top: 1px solid #f1f5f9; display: flex; justify-content: space-between; align-items: center;">
        <div style="font-size: 9px; color: #94a3b8; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">
          Dokumen Internal | Siaga Banjar Core v1.0
        </div>
