import React, { useState, useEffect } from 'react';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from '@/components/ui/dialog';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { ScrollArea } from '@/components/ui/scroll-area';
import { UserPlus, Trash2, Users } from 'lucide-react';
import { EmergencyContact } from '@/types/emergency';
import { toast } from 'sonner';

interface EmergencyContactsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  language?: 'en' | 'id';
}

const EmergencyContactsDialog: React.FC<EmergencyContactsDialogProps> = ({
  open,
  onOpenChange,
  language = 'id',
}) => {
  const t = {
    en: {
      title: "Emergency Contacts",
      description: "Add trusted contacts who will receive your location and status in case of emergency",
      addNew: "Add New Contact",
      name: "Name *",
      namePlaceholder: "Enter contact's full name",
      email: "Email *",
      emailPlaceholder: "contact@example.com",
      phone: "Phone (Optional)",
      phonePlaceholder: "+62 812-xxxx-xxxx",
      relationship: "Relationship (Optional)",
      relationshipPlaceholder: "e.g., Father, Mother, Spouse, Friend",
      addBtn: "Add Contact",
      listTitle: "Your Emergency Contacts",
      noContacts: "No emergency contacts added yet",
      close: "Close",
      errorNameEmail: "Please enter a valid name and email",
      successAdd: "added to emergency contacts",
      successRemove: "Contact removed",
    },
    id: {
      title: "Kontak Darurat",
      description: "Tambahkan kontak terpercaya yang akan menerima lokasi dan status Anda dalam keadaan darurat",
      addNew: "Tambah Kontak Baru",
      name: "Nama *",
      namePlaceholder: "Masukkan nama lengkap kontak",
      email: "Email *",
      emailPlaceholder: "kontak@example.com",
      phone: "Telepon (Opsional)",
      phonePlaceholder: "0812-xxxx-xxxx",
      relationship: "Hubungan (Opsional)",
      relationshipPlaceholder: "misal: Ayah, Ibu, Pasangan, Teman",
      addBtn: "Tambah Kontak",
      listTitle: "Kontak Darurat Anda",
      noContacts: "Belum ada kontak darurat yang ditambahkan",
      close: "Tutup",
      errorNameEmail: "Silakan masukkan nama dan email yang valid",
      successAdd: "ditambahkan ke kontak darurat",
      successRemove: "Kontak dihapus",
    }
  };

  const l = t[language];
  const [contacts, setContacts] = useState<EmergencyContact[]>([]);
  const [newContact, setNewContact] = useState({
    name: '',
    email: '',
    phone: '',
    relationship: '',
  });

  useEffect(() => {
    const saved = localStorage.getItem('emergencyContacts');
    if (saved) {
      setContacts(JSON.parse(saved));
    }
  }, [open]);

  const saveContacts = (updatedContacts: EmergencyContact[]) => {
    localStorage.setItem('emergencyContacts', JSON.stringify(updatedContacts));
    setContacts(updatedContacts);
  };

  const generateId = () => {
    try {
      return crypto.randomUUID();
    } catch {
      return 'id_' + Math.random().toString(36).slice(2, 9);
    }
  };

  const addContact = () => {
    const emailValid = /.+@.+\..+/.test(newContact.email);
    if (!newContact.name || !emailValid) {
      toast.error(l.errorNameEmail);
      return;
    }

    const contact: EmergencyContact = {
      id: generateId(),
      ...newContact,
    };

    const updated = [...contacts, contact];
    saveContacts(updated);
    setNewContact({ name: '', email: '', phone: '', relationship: '' });
    toast.success(`${contact.name} ${l.successAdd}`);
  };

  const removeContact = (id: string) => {
    const updated = contacts.filter(c => c.id !== id);
    saveContacts(updated);
    toast.success(l.successRemove);
  };

  return (
    <Dialog open={open} onOpenChange={onOpenChange} modal={false}>
      <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-lg max-h-[85vh] overflow-y-auto mx-auto z-[3000]" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Users className="h-5 w-5" />
            {l.title}
          </DialogTitle>
          <DialogDescription>
            {l.description}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-3 md:space-y-6">
          {/* Add New Contact Form */}
          <div className="border rounded-lg p-3 md:p-4 space-y-3 md:space-y-4">
            <h3 className="font-semibold flex items-center gap-2">
              <UserPlus className="h-4 w-4" />
              {l.addNew}
            </h3>
            
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 md:gap-4">
              <div className="space-y-2">
                <Label htmlFor="name">{l.name}</Label>
                <Input
                  id="name"
                  value={newContact.name}
                  onChange={(e) => setNewContact({ ...newContact, name: e.target.value })}
                  placeholder={l.namePlaceholder}
                />
              </div>
              
              <div className="space-y-2">
                <Label htmlFor="email">{l.email}</Label>
                <Input
                  id="email"
                  type="email"
                  value={newContact.email}
                  onChange={(e) => setNewContact({ ...newContact, email: e.target.value })}
                  placeholder={l.emailPlaceholder}
                />
              </div>
              
              <div className="space-y-2">
                <Label htmlFor="phone">{l.phone}</Label>
                <Input
                  id="phone"
                  type="tel"
                  value={newContact.phone}
                  onChange={(e) => setNewContact({ ...newContact, phone: e.target.value })}
                  placeholder={l.phonePlaceholder}
                />
              </div>
              
              <div className="space-y-2">
                <Label htmlFor="relationship">{l.relationship}</Label>
                <Input
                  id="relationship"
                  value={newContact.relationship}
                  onChange={(e) => setNewContact({ ...newContact, relationship: e.target.value })}
                  placeholder={l.relationshipPlaceholder}
                />
              </div>
            </div>
            
            <Button onClick={addContact} className="w-full">
              <UserPlus className="h-4 w-4 mr-2" />
              {l.addBtn}
            </Button>
          </div>

          {/* Contacts List */}
          <div className="space-y-2">
            <h3 className="font-semibold">
              {l.listTitle} ({contacts.length})
            </h3>
            
            <ScrollArea className="h-[180px] md:h-[300px] rounded-lg border p-3 md:p-4">
              {contacts.length === 0 ? (
                <div className="text-center text-muted-foreground py-4 md:py-8">
                  <Users className="h-8 w-8 md:h-12 md:w-12 mx-auto mb-2 opacity-50" />
                  <p>{l.noContacts}</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {contacts.map((contact) => (
                    <div
                      key={contact.id}
                      className="flex items-start justify-between p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors"
                    >
                      <div className="flex-1">
                        <div className="font-semibold">{contact.name}</div>
                        <div className="text-sm text-muted-foreground">{contact.email}</div>
                        {contact.phone && (
                          <div className="text-sm text-muted-foreground">{contact.phone}</div>
                        )}
                        {contact.relationship && (
                          <div className="text-xs text-muted-foreground mt-1">
                            {contact.relationship}
                          </div>
                        )}
                      </div>
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => removeContact(contact.id)}
                        className="text-destructive hover:text-destructive"
                      >
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
                  ))}
                </div>
              )}
            </ScrollArea>
          </div>
        </div>

        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>
            {l.close}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
};

export default EmergencyContactsDialog;
