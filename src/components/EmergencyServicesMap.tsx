...
const EmergencyServicesMap: React.FC<EmergencyServicesMapProps> = ({ onFacilityClick, userLocation: dashboardLocation, disasters = [], language = 'id' }) => {
  ...
  const t = {
    ...
    id: {
      searchPlaceholder: "Cari kota untuk layanan...",
      useLocation: "Gunakan Lokasi Saya",
      finding: "Mencari...",
      locationDetected: "Lokasi Terdeteksi",
      locationError: "Kesalahan Lokasi",
      locationNotSupported: "Lokasi Tidak Didukung",
      servicesLoaded: "Layanan Dimuat",
      nearbyServices: "Layanan Terdekat",
      noServices: "Layanan darurat tidak ditemukan",
      getDirections: "Dapatkan Rute",
      routeReady: "Rute Siap",
      cannotShowRoute: "Tidak Bisa Menampilkan Rute",
      routingFailed: "Gagal Membuat Rute",
      hospitals: "Rumah Sakit",
      police: "Polisi",
      fireStations: "Pemadam Kebakaran",
      shelters: "Tempat Evakuasi (Sekolah)",
      temples: "Tempat Ibadah",
      communityHalls: "Balai Desa/Komunitas",
      filterByType: "Filter Berdasarkan Tipe:",
      yourLocation: "Lokasi Anda",
      away: "dari sini",
      tapMarker: "Ketuk penanda untuk petunjuk jalan",
      searchCity: "Cari kota...",
      findingServices: "Mencari layanan...",
    }
  };
  ...
  const getUserLocation = () => {
    ...
    if (!navigator.geolocation) {
      ...
      toast({
        title: l.locationNotSupported,
        description: language === 'en' ? "Your browser doesn't support geolocation. Using default location." : "Browser Anda tidak mendukung geolokasi. Menggunakan lokasi default.",
        variant: "destructive",
      });
      const defaultLoc: [number, number] = [DEFAULT_REGION.center.lat, DEFAULT_REGION.center.lng];
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        let errorMessage = language === 'en' ? "Unable to get your location. " : "Tidak dapat mendapatkan lokasi Anda. ";
        let actionMessage = "";

        switch (error.code) {
          case 1: // PERMISSION_DENIED
            errorMessage = language === 'en' ? "Location permission denied. " : "Izin lokasi ditolak. ";
            actionMessage = language === 'en' ? "Please click the location icon in your browser's address bar and allow location access, then try again." : "Silakan klik ikon lokasi di bilah alamat browser Anda dan izinkan akses lokasi, lalu coba lagi.";
            break;
          case 2: // POSITION_UNAVAILABLE
            errorMessage = language === 'en' ? "Location information unavailable. " : "Informasi lokasi tidak tersedia. ";
            actionMessage = language === 'en' ? "Please check your device's location settings." : "Silakan periksa pengaturan lokasi perangkat Anda.";
            break;
          case 3: // TIMEOUT
            errorMessage = language === 'en' ? "Location request timed out. " : "Permintaan lokasi habis waktu. ";
            actionMessage = language === 'en' ? "Please try again." : "Silakan coba lagi.";
            break;
          default:
            errorMessage = language === 'en' ? "Unknown error occurred." : "Terjadi kesalahan yang tidak diketahui.";
        }

        toast({
          title: l.locationError,
          description: errorMessage + actionMessage + ` Using ${DEFAULT_REGION.name} as default.`,
          variant: "destructive",
          duration: 8000,
        });

        // Default to configured region (Banjarnegara)
        const defaultLoc: [number, number] = [DEFAULT_REGION.center.lat, DEFAULT_REGION.center.lng];
        setUserLocation(defaultLoc);
  ...
  const showRoute = async (service: EmergencyService) => {
    ...
    if (!mapInstanceRef.current || !routeFrom) {
      toast({
        title: l.cannotShowRoute,
        description: language === 'en' ? "User location not available" : "Lokasi pengguna tidak tersedia",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: l.routingFailed,
        description: err.message || (language === 'en' ? "Could not calculate route" : "Tidak dapat menghitung rute"),
        variant: "destructive",
      });
  ...
  // Listen for external routing requests (e.g., from Siaga Banjar AI)
  useEffect(() => {
  ...
  // 📍 Sync with Dashboard's official user location (Handles Manual Override)
  useEffect(() => {
    if (!dashboardLocation || !mapInstanceRef.current) return;
    ...
    // Smoothly pan to the new verified location if it's the first time or manual change
    map.panTo(loc, { animate: true });

    // Sync Marker (Same high-fidelity animated marker as HeatmapOverview)
      ...
      userMarkerRef.current.bindPopup(`
        <div class="p-1">
          <p class="font-bold text-xs uppercase tracking-tight text-primary">
            ${language === 'en' ? 'My Location' : 'Lokasi Terverifikasi'}
          </p>
          <p class="text-[10px] text-muted-foreground mt-0.5">${dashboardLocation.name}</p>
        </div>
      `, { closeButton: false, offset: [0, -5] });
    }

    // Also fetch services for this new location
    fetchNearbyServices(loc[0], loc[1]);
  }, [dashboardLocation, language]);
  ...
  const fetchNearbyServices = async (lat: number, lng: number) => {
    ...
    try {
      ...
      const allFacilities = await fetchEmergencyFacilities(
        { lat, lng, name: '' },
        DEFAULT_REGION.facilitiesRadiusM
      );
      ...
      toast({
        title: l.servicesLoaded,
        description: services.length
          ? (language === 'en' ? `Found ${services.length} nearby emergency services` : `Ditemukan ${services.length} layanan darurat terdekat`)
          : l.noServices,
      });
    } catch (error: any) {
      console.error("Error fetching services:", error);
      toast({
        title: language === 'en' ? "Error loading services" : "Kesalahan memuat layanan",
        description: error.message || (language === 'en' ? "Failed to load emergency services" : "Gagal memuat layanan darurat"),
        variant: "destructive",
      });
      setServices([]);
    } finally {
  ...
  useEffect(() => {
    ...
    if (userLocation) {
      ...
      const userMarker = L.marker(userLocation, { icon: userIcon })
        .bindPopup(`<strong>${l.yourLocation}</strong>`)
        .addTo(mapInstanceRef.current);
    ...
    services.forEach((service, index) => {
      ...
      const typeLabel = service.type === 'hospital' ? l.hospitals : 
                        service.type === 'police' ? l.police :
                        service.type === 'fire_station' ? l.fireStations :
                        service.type === 'school' ? l.shelters :
                        service.type === 'place_of_worship' ? l.temples :
                        service.type === 'community_centre' ? l.communityHalls : service.type;
      ...
      const popupHtml = `
        <div style="min-width: 160px; font-family: system-ui, sans-serif;">
          ...
          >⬆ ${escapeHtml(l.getDirections)}</button>
        </div>
      `;
      ...
      <div className="hidden md:flex w-96 bg-card border-r p-4 flex-col h-full overflow-y-auto z-10 relative">
        <div className="mb-4 space-y-4">
          ...
          <Button onClick={getUserLocation} disabled={loading} className="w-full" variant="outline">
            {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> {l.finding}</> : <><Navigation className="w-4 h-4 mr-2" /> {l.useLocation}</>}
          </Button>
        </div>

        <div className="mb-4 space-y-2">
          <p className="font-semibold text-sm">{l.filterByType}</p>
          {[
            { id: "hospital", label: l.hospitals, icon: <Hospital className="w-4 h-4" /> },
            { id: "police", label: l.police, icon: <Shield className="w-4 h-4" /> },
            { id: "fire_station", label: l.fireStations, icon: <Flame className="w-4 h-4" /> },
            { id: "school", label: l.shelters, icon: <School className="w-4 h-4" /> },
            { id: "place_of_worship", label: l.temples, icon: <Church className="w-4 h-4" /> },
            { id: "community_centre", label: l.communityHalls, icon: <Tent className="w-4 h-4" /> },
          ].map(({ id, label, icon }) => (
        ...
        <div className="space-y-2 flex-1 overflow-y-auto pb-4">
          <p className="font-semibold text-xs mb-1.5 sticky top-0 bg-card z-10 py-1">{l.nearbyServices} ({services.length})</p>
          {loading && (
          ...
          {!loading && services.length === 0 && <div className="text-center py-4"><p className="text-[10px] text-muted-foreground">{l.noServices}</p></div>}
            ...
            <Card key={service.id} className={`p-2.5 hover:bg-accent cursor-pointer transition-all ${selectedService?.id === service.id ? 'ring-2 ring-primary' : ''}`} onClick={() => mapInstanceRef.current?.setView([service.lat, service.lng], 15)}>
              <div className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground">{service.distance.toFixed(2)} km {l.away}</p>
                  {service.address && <p className="text-[10px] text-muted-foreground mt-0.5 line-clamp-1">{service.address}</p>}
                  <Button size="sm" variant="outline" className="mt-1.5 w-full h-7 text-[11px]" onClick={(e) => { e.stopPropagation(); showRoute(service); }}><Navigation className="w-3 h-3 mr-1" />{l.getDirections}</Button>
                </div>
              </div>
            </Card>
          ))}
        ...
        <div className="pointer-events-auto p-3 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1.5 overflow-x-auto hide-scrollbar">
              {[
                { id: "hospital", label: l.hospitals.split(' ')[0], icon: <Hospital className="w-3 h-3" /> },
                { id: "police", label: l.police, icon: <Shield className="w-3 h-3" /> },
                { id: "fire_station", label: language === 'en' ? "Fire" : "Damkar", icon: <Flame className="w-3 h-3" /> },
                { id: "school", label: language === 'en' ? "Shelter" : "Evakuasi", icon: <School className="w-3 h-3" /> },
                { id: "place_of_worship", label: language === 'en' ? "Temple" : "Ibadah", icon: <Church className="w-3 h-3" /> },
                { id: "community_centre", label: language === 'en' ? "Community" : "Balai Desa", icon: <Tent className="w-3 h-3" /> },
              ].map(({ id, label, icon }) => (
                <button
                  key={id}
          ...
          <div className="pointer-events-auto flex items-center justify-center py-2">
            <div className="glass-strong bg-background/90 backdrop-blur-xl rounded-2xl px-4 py-3 shadow-lg border border-white/20 w-[90%] max-w-xs space-y-2">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-semibold">{l.findingServices}</span>
              </div>
              {[1, 2, 3].map((i) => (
        ...
        <div className="pointer-events-auto w-full pb-[72px] px-3">
          {!loading && services.length > 0 && !selectedService && (
            <div className="glass-strong bg-background/90 backdrop-blur-xl rounded-xl shadow-lg border border-white/20 px-3 py-2.5 flex items-center justify-between">
              <span className="text-[11px] font-semibold text-foreground">{services.length} {language === 'en' ? 'services found' : 'layanan ditemukan'}</span>
              <span className="text-[9px] text-muted-foreground">{l.tapMarker}</span>
            </div>
          )}
            ...
            <Card className="glass-strong backdrop-blur-xl bg-background/90 shadow-2xl border border-white/20 rounded-xl p-2.5 animate-in slide-in-from-bottom-4 duration-300">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[9px] text-primary font-medium mt-0.5">{selectedService.distance.toFixed(2)} km {l.away}</p>
