...
const WeatherWidget: React.FC<WeatherWidgetProps> = ({ weather, loading, onLocationChange, userLocation, language = 'id' }) => {
  ...
  if (!weather) {
    return (
      <Card className="bg-sky-50/50 dark:bg-slate-900/50 border-2 border-sky-100 dark:border-slate-800 p-8 text-center">
        ...
        <h3 className="font-semibold text-lg mb-2 text-sky-900 dark:text-sky-100">{language === 'en' ? 'Weather Data Unavailable' : 'Data Cuaca Tidak Tersedia'}</h3>
        <p className="text-muted-foreground text-sm">
          {language === 'en' ? 'Unable to fetch weather information. Please check your location settings.' : 'Tidak dapat mengambil informasi cuaca. Silakan periksa pengaturan lokasi Anda.'}
        </p>
      </Card>
    );
  }
  ...
  const getWindDirection = (deg?: number) => {
    ...
    const directionsId = ['U', 'UTL', 'TL', 'TTL', 'T', 'TTG', 'TG', 'STG', 'S', 'SBD', 'BD', 'BBD', 'B', 'BBT', 'BT', 'UBL'];
    const directions = language === 'en' ? directionsEn : directionsId;
    return directions[Math.round(deg / 22.5) % 16];
  };

  const formatTime = (timestamp: number) => {
    return new Date(timestamp * 1000).toLocaleTimeString(language === 'en' ? 'en-US' : 'id-ID', {
      hour: '2-digit',
      minute: '2-digit',
      hour12: true
    });
  ...
  const getUVLabel = (uv?: number) => {
    if (!uv) return 'N/A';
    if (uv < 3) return language === 'en' ? 'Low' : 'Rendah';
    if (uv < 6) return language === 'en' ? 'Moderate' : 'Sedang';
    if (uv < 8) return language === 'en' ? 'High' : 'Tinggi';
    if (uv < 11) return language === 'en' ? 'Very High' : 'Sangat Tinggi';
    return language === 'en' ? 'Extreme' : 'Ekstrem';
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <Card className="bg-card/50 backdrop-blur-md border-2 border-border p-3 sm:p-4 shadow-sm">
        <div className="space-y-3">
          <div ref={searchContainerRef} className="relative">
            ...
            <Input
              type="text"
              placeholder={language === 'en' ? "Search city to view weather..." : "Cari kota untuk melihat cuaca..."}
              value={searchQuery}
            ...
            {searchQuery.trim() && !isSearching && searchResults.length === 0 && showDropdown && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-lg z-50 p-3">
                <p className="text-sm text-muted-foreground text-center">{language === 'en' ? 'No cities found' : 'Kota tidak ditemukan'}</p>
              </div>
            )}
          </div>

          {weather && (userLocation || weather.bmkgMeta) && (
            <p className="text-xs text-muted-foreground text-center">
              Prakiraan untuk: {weather.location.name || `${weather.location.lat.toFixed(4)}, ${weather.location.lng.toFixed(4)}`}
            </p>
          )}
        </div>
      </Card>
      ...
      {weather.bmkgMeta && (
        <Card className="bg-card/80 backdrop-blur-md border border-border/60 p-3 sm:p-4 shadow-sm">
          <p className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed">
            <span className="font-semibold text-foreground">Sumber data BMKG:</span>{' '}
            Prakiraan cuaca (adm4 {weather.bmkgMeta.adm4}) dan peringatan dini nowcast diintegrasikan sesuai{' '}
            <a
            ...
            . Referensi contoh pengolahan JSON:{' '}
            <a
              className="text-primary underline underline-offset-2"
              href="https://github.com/infoBMKG/data-cuaca"
              target="_blank"
              rel="noreferrer"
            >
              infoBMKG/data-cuaca
      ...
      <Card className="bg-card backdrop-blur-md border-2 border-border p-4 sm:p-6 shadow-md shadow-primary/5">
        <div className="flex items-start justify-between mb-4 sm:mb-6">
          <div className="min-w-0">
            <h2 className="text-lg sm:text-2xl font-black text-foreground flex items-center gap-2 tracking-tight uppercase">
              ...
              <span className="truncate">{language === 'en' ? 'Climate Intelligence' : 'Intelijen Iklim'}</span>
            </h2>
          ...
          <div className="text-right flex-shrink-0 ml-3">
            ...
            {weather.feelsLike && (
              <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">
                {language === 'en' ? `Feels like ${weather.feelsLike}°C` : `Terasa seperti ${weather.feelsLike}°C`}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4 mb-4 sm:mb-6">
          <div className="flex items-center gap-3 p-4 rounded-xl bg-sky-100/30 dark:bg-slate-800/40 border border-sky-100/50 dark:border-slate-700/30 transition-all hover:bg-sky-100/50 dark:hover:bg-slate-800/60">
            ...
            <div>
              <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider">{language === 'en' ? 'Humidity' : 'Kelembapan'}</p>
          ...
          <div className="flex items-center gap-3 p-4 rounded-xl bg-sky-100/30 dark:bg-slate-800/40 border border-sky-100/50 dark:border-slate-700/30 transition-all hover:bg-sky-100/50 dark:hover:bg-slate-800/60">
            ...
            <div>
              <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider">{language === 'en' ? 'Wind Velocity' : 'Kecepatan Angin'}</p>
          ...
          <div className="flex items-center gap-3 p-4 rounded-xl bg-sky-100/30 dark:bg-slate-800/40 border border-sky-100/50 dark:border-slate-700/30 transition-all hover:bg-sky-100/50 dark:hover:bg-slate-800/60">
            ...
            <div>
              <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider">{language === 'en' ? 'Precipitation' : 'Presipitasi'}</p>
          ...
          {weather.visibility !== undefined && (
            <div className="flex items-center gap-3 p-4 rounded-xl bg-sky-100/30 dark:bg-slate-800/40 border border-sky-100/50 dark:border-slate-700/30 transition-all hover:bg-sky-100/50 dark:hover:bg-slate-800/60">
              ...
              <div>
                <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider">{language === 'en' ? 'Visibility' : 'Jarak Pandang'}</p>
          ...
          {weather.pressure && (
            <div className="flex items-center gap-3 p-3 rounded-lg bg-sky-100/30 dark:bg-slate-800/40 border border-sky-100/50 dark:border-slate-700/30">
              ...
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">{language === 'en' ? 'Pressure' : 'Tekanan'}</p>
          ...
          {weather.sunrise && (
            <div className="flex items-center gap-3 p-3 rounded-lg bg-sky-100/30 dark:bg-slate-800/40 border border-sky-100/50 dark:border-slate-700/30">
              ...
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">{language === 'en' ? 'Sunrise' : 'Matahari Terbit'}</p>
          ...
          {weather.sunset && (
            <div className="flex items-center gap-3 p-3 rounded-lg bg-sky-100/30 dark:bg-slate-800/40 border border-sky-100/50 dark:border-slate-700/30">
              ...
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">{language === 'en' ? 'Sunset' : 'Matahari Terbenam'}</p>
          ...
          <div className="flex items-center gap-3 p-3 rounded-lg bg-sky-100/30 dark:bg-slate-800/40 border border-sky-100/50 dark:border-slate-700/30">
            ...
            <div>
              <p className="text-xs text-slate-500 dark:text-slate-400">{language === 'en' ? 'Wind Dir.' : 'Arah Angin'}</p>
            ...
            <div className={`p-4 rounded-xl border ${getUVColor(weather.uvIndex)}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">{language === 'en' ? 'UV Index' : 'Indeks UV'}</span>
                </div>
            ...
            <div className={`p-4 rounded-xl border ${getAirQualityColor(weather.airQuality.aqi)}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">{language === 'en' ? 'Air Quality' : 'Kualitas Udara'}</span>
                </div>
        ...
        <div className="mt-4 p-4 rounded-xl bg-gradient-to-r from-sky-50 to-white dark:from-slate-800/50 dark:to-slate-900/50 border border-sky-100 dark:border-slate-800">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-black uppercase text-sky-600 dark:text-sky-400">
              {weather.temperature > 35 ? (language === 'en' ? 'Very Hot' : 'Sangat Panas') :
                weather.temperature > 30 ? (language === 'en' ? 'Hot' : 'Panas') :
                  weather.temperature > 25 ? (language === 'en' ? 'Perfect' : 'Sempurna') :
                    weather.temperature > 15 ? (language === 'en' ? 'Cool' : 'Sejuk') : (language === 'en' ? 'Cold' : 'Dingin')}
            </span>
          </div>
          <Progress
      ...
      {weather.hourlyForecast && weather.hourlyForecast.length > 0 && (
        <Card className="bg-card backdrop-blur-md border-2 border-border p-4 sm:p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-4 sm:mb-6">
            ...
            <h3 className="font-black text-xs sm:text-sm uppercase tracking-[0.2em] text-foreground">{language === 'en' ? 'Near-Term Forecast' : 'Prakiraan Jangka Pendek'}</h3>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={weather.hourlyForecast.map(hour => ({
                ...
                <YAxis
                  stroke="hsl(var(--muted-foreground))"
                  fontSize={12}
                  label={{ value: language === 'en' ? 'Temperature (°C)' : 'Suhu (°C)', angle: -90, position: 'insideLeft' }}
                />
                <Tooltip
                  contentStyle={{
      ...
      {weather.alerts.length > 0 && (
        <Card className="p-6 border-2 bg-red-50/50 dark:bg-red-950/20 border-red-200 dark:border-red-900/30">
          <div className="flex items-center gap-2 mb-6">
            ...
            <div>
              <h3 className="font-black text-sm uppercase tracking-widest text-red-900 dark:text-red-100">{language === 'en' ? 'Critical Alerts' : 'Peringatan Kritis'}</h3>
              <p className="text-[10px] uppercase font-bold text-red-600/60 dark:text-red-400/60">
                {weather.alerts.length} {language === 'en' ? `active warning${weather.alerts.length > 1 ? 's' : ''}` : `peringatan aktif`}
              </p>
            </div>
          </div>
            ...
            {weather.alerts.map((alert) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3 p-3 rounded-lg bg-red-100/30 dark:bg-red-900/20">
                  <div>
                    <p className="text-[10px] mb-1 font-bold uppercase text-red-900/40 dark:text-red-100/40">{language === 'en' ? 'Starts' : 'Mulai'}</p>
                    <p className="text-xs font-bold text-red-900 dark:text-red-100">{new Date(alert.start).toLocaleString(language === 'en' ? 'en-IN' : 'id-ID')}</p>
                  </div>
                  <div>
                    <p className="text-[10px] mb-1 font-bold uppercase text-red-900/40 dark:text-red-100/40">{language === 'en' ? 'Ends' : 'Selesai'}</p>
                    <p className="text-xs font-bold text-red-900 dark:text-red-100">{new Date(alert.end).toLocaleString(language === 'en' ? 'en-IN' : 'id-ID')}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <Card className="bg-card backdrop-blur-md border-2 border-border p-4 sm:p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-4 sm:mb-6">
          ...
          <h3 className="font-black text-xs sm:text-sm uppercase tracking-[0.2em] text-foreground">{language === 'en' ? 'Weekly Outlook' : 'Pandangan Mingguan'}</h3>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 sm:gap-4">
          {weather.forecast.slice(0, 5).map((day, index) => (
            ...
            >
              <p className="text-xs font-bold text-sky-900 dark:text-sky-50 mb-2 uppercase tracking-wide">
                {new Date(day.date).toLocaleDateString(language === 'en' ? 'en-IN' : 'id-ID', { weekday: 'short', month: 'short', day: 'numeric' })}
              </p>
