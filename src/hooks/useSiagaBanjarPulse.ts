...
interface SiagaBanjarPulseState {
  riskScore: number;
  activeDisasters: any[];
...
/**
 * SiagaBanjar Pulse Hook
 * Monitors global disaster telemetry and triggers autonomous AI interjections
 * when critical risk thresholds are met.
 */
export const useSiagaBanjarPulse = (userLocation: Location | null) => {
  const [pulse, setPulse] = useState<SiagaBanjarPulseState>({
    riskScore: 0,
    activeDisasters: [],
    ...
    try {
      ...
      if (score >= 40 && !pulse.isInterjectionNeeded) {
        ...
        setPulse(prev => ({
          ...
          message: `Saya mendeteksi aktivitas seismik yang signifikan di dekat ${primaryEvent}. Apakah Anda ingin ringkasan AI segera?`
        }));
      } else {
