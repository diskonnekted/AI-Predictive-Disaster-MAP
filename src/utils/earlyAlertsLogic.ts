...
export async function fetchEarlyAlertsLocal(
    ...
    if (mlResult) {
      ...
      if (prob >= 0.85) {
        alerts.push({
          ...
          type: "flood",
          severity: "emergency",
          title: language === 'en' ? "🚨 ML Flood Model: Extreme Risk Detected" : "🚨 Model ML Banjir: Risiko Ekstrim Terdeteksi",
          description: language === 'en' 
            ? `TF.js Neural Network predicts ${(prob * 100).toFixed(0)}% flood probability. 24h precip: ${forecast.precip24h.toFixed(1)}mm, peak rate: ${forecast.maxHourlyRate.toFixed(1)}mm/h. Evacuate low-lying areas immediately.`
            : `Jaringan Saraf TF.js memprediksi probabilitas banjir ${(prob * 100).toFixed(0)}%. Presipitasi 24 jam: ${forecast.precip24h.toFixed(1)}mm, laju puncak: ${forecast.maxHourlyRate.toFixed(1)}mm/h. Segera evakuasi area rendah.`,
          source: language === 'en' ? "TensorFlow.js Neural Network (trained on 2500 regional flood samples)" : "Jaringan Saraf TensorFlow.js (dilatih pada 2500 sampel banjir regional)",
        ...
        alerts.push({
          ...
          type: "flood",
          severity: "warning",
          title: language === 'en' ? "⚠️ ML Flood Model: High Risk" : "⚠️ Model ML Banjir: Risiko Tinggi",
          description: language === 'en'
            ? `Neural Network flood probability: ${(prob * 100).toFixed(0)}%. Model AUC-ROC: ${mlResult.metrics.auc_roc}. Monitor drainage and water levels.`
            : `Probabilitas banjir Jaringan Saraf: ${(prob * 100).toFixed(0)}%. AUC-ROC Model: ${mlResult.metrics.auc_roc}. Pantau drainase dan ketinggian air.`,
          source: language === 'en' ? "TensorFlow.js Neural Network (trained on 2500 regional flood samples)" : "Jaringan Saraf TensorFlow.js (dilatih pada 2500 sampel banjir regional)",
          algorithm: mlResult.modelInfo,
          dataPoints: { ...mlResult.features, probability: prob },
          location: { lat, lng },
        ...
        alerts.push({
          ...
          type: "flood",
          severity: "watch",
          title: language === 'en' ? "🌧️ ML Flood Model: Moderate Risk" : "🌧️ Model ML Banjir: Risiko Sedang",
          description: language === 'en'
            ? `Neural Network flood probability: ${(prob * 100).toFixed(0)}%. Stay aware of conditions.`
            : `Probabilitas banjir Jaringan Saraf: ${(prob * 100).toFixed(0)}%. Tetap waspada terhadap kondisi sekitar.`,
          source: language === 'en' ? "TensorFlow.js Neural Network (trained on 2500 regional flood samples)" : "Jaringan Saraf TensorFlow.js (dilatih pada 2500 sampel banjir regional)",
          algorithm: mlResult.modelInfo,
          dataPoints: { ...mlResult.features, probability: prob },
          location: { lat, lng },
    ...
    if (mlResult) {
      ...
      if (prob >= 0.70) {
        alerts.push({
          ...
          type: "earthquake",
          severity: "emergency",
          title: language === 'en' ? "🚨 ML Earthquake Model: Extreme Risk" : "🚨 Model ML Gempa: Risiko Ekstrim",
          description: language === 'en'
            ? `TF.js Neural Network predicts ${(prob * 100).toFixed(0)}% probability of an M5.0+ event within 30 days. Model AUC-ROC: ${mlResult.metrics?.auc_roc || "N/A"}. High seismic activity detected.`
            : `Jaringan Saraf TF.js memprediksi probabilitas ${(prob * 100).toFixed(0)}% untuk kejadian M5.0+ dalam 30 hari. AUC-ROC Model: ${mlResult.metrics?.auc_roc || "N/A"}. Aktivitas seismik tinggi terdeteksi.`,
          source: language === 'en' ? "TensorFlow.js Neural Network (trained on 2000 regional seismic samples)" : "Jaringan Saraf TensorFlow.js (dilatih pada 2000 sampel seismik regional)",
          algorithm: mlResult.modelInfo,
          dataPoints: { ...mlResult.features, probability: prob },
          location: { lat, lng },
          issuedAt: now,
        ...
        alerts.push({
          ...
          type: "earthquake",
          severity: "warning",
          title: language === 'en' ? "⚠️ ML Earthquake Model: Elevated Risk" : "⚠️ Model ML Gempa: Risiko Meningkat",
          description: language === 'en'
            ? `TF.js Neural Network predicts ${(prob * 100).toFixed(0)}% probability of significant seismic event.`
            : `Jaringan Saraf TF.js memprediksi probabilitas ${(prob * 100).toFixed(0)}% untuk kejadian seismik signifikan.`,
  ...
  if (weather) {
    ...
    if (heatIndex >= 54) {
      alerts.push({
        ...
        type: "heatwave",
        severity: "emergency",
          title: language === 'en' ? "🔥 Extreme Heat Emergency" : "🔥 Darurat Panas Ekstrim",
          description: language === 'en'
            ? `Heat index: ${heatIndex.toFixed(0)}°C (actual: ${temp}°C, RH: ${humidity}%). Heatstroke imminent.`
            : `Indeks panas: ${heatIndex.toFixed(0)}°C (aktual: ${temp}°C, Kelembapan: ${humidity}%). Risiko heatstroke tinggi.`,
          source: "Open-Meteo + Steadman Heat Index",
          algorithm: language === 'en' ? "Steadman (1979) HI formula. Threshold: 54°C." : "Rumus HI Steadman (1979). Ambang batas: 54°C.",
    ...
    if (temp <= 4) {
      alerts.push({
        ...
        type: "cold_wave",
        severity: "warning",
          title: language === 'en' ? "🥶 Cold Wave Warning" : "🥶 Peringatan Gelombang Dingin",
          description: language === 'en'
            ? `Temperature: ${temp}°C (feels like ${feelsLike}°C). Local cold threshold breached.`
            : `Suhu: ${temp}°C (terasa seperti ${feelsLike}°C). Ambang batas dingin lokal terlampaui.`,
          source: language === 'en' ? "Open-Meteo + Regional Guidelines" : "Open-Meteo + Pedoman Regional",
          algorithm: language === 'en' ? "Local threshold: T ≤ 10°C in tropical highlands." : "Ambang batas lokal: T ≤ 10°C di dataran tinggi tropis.",
          dataPoints: { temp, feelsLike, threshold: 10 },
        location: { lat, lng },
        issuedAt: now,
        expiresAt: new Date(Date.now() + 24 * 3600000).toISOString(),
        confidence: 0.9,
    ...
    if (windKmh >= 118) {
      alerts.push({
        ...
        type: "cyclone",
        severity: "emergency",
          title: language === 'en' ? "🌀 Hurricane-Force Winds" : "🌀 Angin Kencang / Puting Beliung",
          description: language === 'en'
            ? `Wind: ${windKmh.toFixed(0)} km/h (Beaufort 12). Take shelter.`
            : `Angin: ${windKmh.toFixed(0)} km/h (Skala Beaufort 12). Segera cari perlindungan.`,
    ...
    if (condLower.includes("thunderstorm")) {
      alerts.push({
        ...
        severity: windKmh > 60 ? "warning" : "watch",
          title: language === 'en' ? "⛈️ Thunderstorm Alert" : "⛈️ Peringatan Badai Petir",
          description: language === 'en'
            ? `Active thunderstorm. Wind: ${windKmh.toFixed(0)} km/h. Avoid open areas.`
            : `Badai petir aktif. Angin: ${windKmh.toFixed(0)} km/h. Hindari area terbuka.`,
          source: "Open-Meteo",
          algorithm: language === 'en' ? "Weather code classification." : "Klasifikasi kode cuaca.",
    ...
    if (
      ...
      alerts.push({
        ...
        severity: pressure < 980 ? "warning" : "advisory",
          title: language === 'en' ? "🌀 Low Pressure System" : "🌀 Sistem Tekanan Rendah",
          description: language === 'en'
            ? `Pressure: ${pressure} hPa, wind: ${windKmh.toFixed(0)} km/h. Storm possible.`
            : `Tekanan: ${pressure} hPa, angin: ${windKmh.toFixed(0)} km/h. Potensi badai.`,
          source: "Open-Meteo",
          algorithm: language === 'en' ? "Low pressure (<1000 hPa) + wind (>50 km/h)." : "Tekanan rendah (<1000 hPa) + angin (>50 km/h).",
  ...
  return {
    ...
    metadata: {
      ...
      algorithmsUsed: [
        language === 'en' 
          ? "TF.js Neural Network Flood Model (trained on 2500 regional flood samples, AUC-ROC ~0.99)" 
          : "Model Jaringan Saraf TF.js untuk Banjir (dilatih pada 2500 sampel banjir regional, AUC-ROC ~0.99)",
        language === 'en' 
          ? "TF.js Neural Network Earthquake Risk Model (trained on 2000 regional seismic samples, AUC-ROC ~0.85)" 
          : "Model Risiko Gempa Jaringan Saraf TF.js (dilatih pada 2000 sampel seismik regional, AUC-ROC ~0.85)",
      ],
    },
  };
}
